    ...
    >
      ...
      {currentModule && (
        <div className="animate-fade-in" style={{ marginBottom: 10 }}>
          <p style={{
            ...
            {currentModule.title}
            {currentSection && (
              <span style={{ color: "rgba(var(--color-ep-light-rgb),0.25)", fontWeight: 500, textTransform: "none", letterSpacing: "0.04em" }}>
      ...
      >
        {prevLesson ? (
          <Link
            ...
            style={{
              ...
              border: "1px solid rgba(var(--color-ep-red-rgb),0.12)",
              borderRadius: "var(--radius-lg)",
              textDecoration: "none",
            }}
          >
            <ChevronLeft size={15} style={{ color: "rgba(var(--color-ep-light-rgb),0.3)", flexShrink: 0 }} />
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(var(--color-ep-light-rgb),0.25)", margin: "0 0 2px" }}>
                Précédent
              </p>
              <p style={{
                fontSize: 11, fontWeight: 700, color: "rgba(var(--color-ep-light-rgb),0.65)", margin: 0,
        ...
        {nextLesson ? (
          ...
          >
            ...
            <ChevronRight size={15} style={{ color: "rgba(var(--color-ep-light-rgb),0.3)", flexShrink: 0 }} />
          </Link>
        ) : (
          <Link
            href={`/dashboard/client/formations/${formationId}`}
      ...
      {currentSection && currentSection.lessons.length > 1 && (
        <div className="ep-card animate-fade-up" style={{ overflow: "hidden" }}>
          <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(var(--color-ep-red-rgb),0.08)" }}>
            <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(var(--color-ep-red-rgb),0.4)", margin: "0 0 1px" }}>
              {currentModule?.title}
            </p>
            <p style={{ fontSize: 11, fontWeight: 700, color: "rgba(var(--color-ep-light-rgb),0.6)", margin: 0 }}>
              {currentSection.title}
            </p>
          </div>
          {currentSection.lessons.map((l, i) => {
            ...
            return (
              <div key={l.id}>
                {i > 0 && <div style={{ height: 1, background: "rgba(var(--color-ep-red-rgb),0.05)", margin: "0 16px" }} />}
                {isAvail ? (
                  <Link
                    ...
                    style={{
                      ...
                      background: isCurrent ? "rgba(var(--color-ep-red-rgb),0.06)" : "transparent",
                      borderLeft: isCurrent ? "2px solid var(--color-ep-red)" : "2px solid transparent",
                    }}
                  >
                    <span style={{ fontSize: 11, fontWeight: 600, color: "rgba(var(--color-ep-light-rgb),0.2)", width: 18, flexShrink: 0, textAlign: "right" }}>
                      {i + 1}
                    </span>
                    <span style={{
                      fontSize: 12, fontWeight: isCurrent ? 700 : 500,
                      color: isCurrent ? "var(--color-ep-light)" : isDone ? "rgba(var(--color-ep-light-rgb),0.4)" : "rgba(var(--color-ep-light-rgb),0.65)",
                      flex: 1, lineHeight: 1.3,
                    }}>
                      {l.title}
                    </span>
                    {isCurrent && (
                      <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--color-ep-red)", flexShrink: 0 }} />
                    )}
                  </Link>
                ) : (
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", opacity: 0.35 }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: "rgba(var(--color-ep-light-rgb),0.2)", width: 18, textAlign: "right" }}>
                      {i + 1}
                    </span>
                    <span style={{ fontSize: 12, color: "rgba(var(--color-ep-light-rgb),0.4)", flex: 1 }}>{l.title}</span>
                  </div>
                )}
              </div>
            );
