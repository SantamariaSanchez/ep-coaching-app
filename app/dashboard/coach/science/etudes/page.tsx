...
export default async function CoachScienceEtudesPage() {
  const user = await getUser();
  if (!user) redirect("/");

  const profile = await getProfile(user.id);
  if (profile?.role === "client") redirect("/dashboard/client/science/etudes");

  const [studies, counts] = await Promise.all([getScienceStudies(user.id), getScienceCounts(user.id)]);
    ...
    <div className="px-6 py-8 max-w-2xl mx-auto pb-24 md:pb-8 page-transition">
      <div className="mb-2">
        ...
        <p className="mt-1 text-sm text-[#F5EDED]/40">
          Mène tes propres protocoles avec la communauté EP Coaching.
        </p>
      </div>
