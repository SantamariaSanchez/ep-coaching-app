...
function AvgRow({ label, value, unit = "" }: { label: string; value: number | null; unit?: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid rgba(var(--color-ep-dark-red-rgb),0.08)" }}>
...
export default async function CoachMonBilanPage() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 16px 80px" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(var(--color-ep-dark-red-rgb),0.2)", borderRadius: 16, padding: "20px 16px", marginBottom: 32 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {weeks.map(({ weekStart, logs, averages }) => (
              <div key={weekStart}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, borderBottom: "1px solid rgba(var(--color-ep-dark-red-rgb),0.15)", paddingBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 9, color: "rgba(var(--color-ep-light-rgb),0.2)", fontWeight: 600 }}>{logs.length} jour{logs.length > 1 ? "s" : ""}</span>
                </div>
                <div style={{ background: "rgba(0,0,0,0.25)", border: "1px solid rgba(var(--color-ep-dark-red-rgb),0.1)", borderRadius: 10, padding: "10px 14px", marginBottom: 10 }}>
                  <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(var(--color-ep-red-rgb),0.4)", margin: "0 0 8px" }}>Moyennes</p>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {logs.map((log) => (
                    <div key={log.id} style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(var(--color-ep-dark-red-rgb),0.12)", borderRadius: 10, padding: "10px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                        <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(var(--color-ep-red-rgb),0.5)" }}>{fmtShort(log.log_date)}</span>
                        {log.training_name && <span style={{ fontSize: 11, fontWeight: 700, color: "var(--color-ep-light)" }}>{log.training_name}</span>}
                      </div>
