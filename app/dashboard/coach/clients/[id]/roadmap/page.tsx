...
import { ChevronLeft, Plus, Trash2, Save, Eye, EyeOff, Target, CalendarRange } from "lucide-react";
...
function ObjectiveCard({
  ...
  const termColor = OBJECTIVE_TERM_COLORS[obj.term];
    ...
    <div style={{ padding: "24px 24px 64px", maxWidth: 900, margin: "0 auto" }}>
        ...
        {phases.length === 0 && (
          <div className="ep-card" style={{ padding: 32, textAlign: "center" }}>
            <p style={{ color: "rgba(245,237,237,0.3)", fontSize: 13 }}>
              Aucune phase — clique sur &laquo; Ajouter une phase &raquo;
            </p>
          </div>
        )}
      ...
      <div style={{ marginTop: 32, display: "flex", justifyContent: "flex-end" }}>
        <button onClick={handleSave} disabled={saving} className="ep-btn-primary" style={{ fontSize: 14, padding: "14px 32px" }}>
          <Save size={16} />
          {saving ? "Sauvegarde en cours…" : "Sauvegarder la road map"}
        </button>
      </div>
    </div>
  );
...
// Local inline PhaseTimelineBar (not imported — avoid circular)
