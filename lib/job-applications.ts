...
//
// Les questions de qualification et types partagés avec le formulaire public
// (Client Component) vivent dans job-applications-shared.ts, qui n'importe
// jamais createAdminClient — voir le commentaire là-bas. Réexportés ici pour
// que le code existant qui importe depuis "@/lib/job-applications" continue
...
export async function getJobApplications(ownerId: string): Promise<JobApplication[]> {
  try {
    ...
    const { data } = await admin
      ...
      .select("id, owner_id, role_key, full_name, email, phone, status, notes, answers, created_at")
      .eq("owner_id", ownerId)
