...
export default function ClientTasksView() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      // Session expirée depuis le chargement de la page (garde serveur déjà
      // passée au premier accès) : on renvoie vers la connexion au lieu de
      // rester bloqué sur le skeleton indéfiniment.
      if (!user) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 28 }}>
          {pending.map((task) => (
            ...
            >
              ...
              <span style={{ flex: 1, fontSize: 15, fontWeight: 700, color: "#F5EDED" }}>
                {task.label}
              </span>
              {completing === task.id ? (
                <div
                  className="animate-spin"
                  style={{ width: 18, height: 18, border: "2px solid #E01E1E", borderTopColor: "transparent", borderRadius: "50%" }}
                />
              ) : (
                <Circle size={20} style={{ color: "rgba(224,30,30,0.4)" }} strokeWidth={2} />
              )}
            </button>
          ))}
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {done.slice(0, 15).map((task) => (
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: 13, color: "#F5EDED", textDecoration: "line-through" }}>
                  {task.label}
                </span>
