...
function ExerciseProgression({ sessions, records }: { sessions: SessionWithSets[]; records: PersonalRecord[] }) {
  const [selectedExercise, setSelectedExercise] = useState<string | null>(null);

  // Build exercise list from all sessions
        ...
        <div className="flex items-center gap-3 mb-4 p-3 bg-amber-500/5 border border-amber-500/15 rounded-xl">
          ...
          <p className="ml-auto text-[9px] text-[#F5EDED]/35">
            {new Intl.DateTimeFormat("fr-FR", {
              day: "numeric",
              month: "short",
            }).format(new Date(bestRecord.achieved_at + "T12:00:00"))}
          </p>
        </div>
      )}
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {recentTableData.map((row, i) => (
                <tr
                  key={i}
                  className="border-t border-[#890404]/10"
                >
                  ...
                  <td className="py-2 pr-3 text-[#F5EDED]/60">
                    {row.reps ?? "—"}
                  </td>
      ...
      {activeTab === "progression" && (
        <div className="space-y-6">
          <section>
            ...
            <ExerciseProgression sessions={sessions} records={records} />
          </section>
        </div>
      )}
